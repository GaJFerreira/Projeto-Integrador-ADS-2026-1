...
import br.pucgo.ads.projetointegrador.eldencare.exception.NotFoundException;
...
public class PlanPersistenceService {
    ...
    // gera um plano básico (3 dias com 1 item cada) a partir do participante
    @Transactional
    public ex_plano gerarPlanoBasico(UUID participanteId, String objetivo, String nivel,
        ...
        ex_plano plano = buildHeader(participante, null, objetivo, nivel, freqSemana, tempoSessaoMin);
    ...
    // gera plano vinculando um response_id existente (DER oficial)
    @Transactional
    ...
    // adiciona 1 item a um plano existente; cria o dia se ainda não existir
    @Transactional
    ...
    private ex_dia_plano ensureDia(ex_plano plano, String diaLabel) {
        for (ex_dia_plano d : plano.getDias()) {
            if (diaLabel.equalsIgnoreCase(d.getDataOuOrdem())) return d;
        }
