...
public interface UsuarioConquistaRepository extends JpaRepository<UsuarioConquista, UsuarioConquistaKey> {
    ...
    @Query("SELECT u.nome AS nomeUsuario, SUM(c.pontos) AS totalPontos FROM UsuarioConquista uc JOIN uc.usuario u JOIN uc.conquista c GROUP BY u.idUsuario, u.nome ORDER BY SUM(c.pontos) DESC")
