...
const CadastroMedicamento: React.FC<Props> = ({ medicamentoEditar }) => {
    ...
    useEffect(() => {
        if (medicamentoEditar) {
            setAnvisaSelecionada(null); // será resolvido pelo anvisaId no edit se necessário
    ...
    const handleSubmit = async (e?: FormEvent) => {
        ...
        const payload = montarPayload();  // usa o usuarioId REAL
                        ...
                        {step === 1 && (
                            <Stack spacing={1} sx={{ mt: 1 }}>
                                ...
                                <Autocomplete
                                    ...
                                    value={anvisaSelecionada}
                                    inputValue={anvisaInputValue}
                                    onInputChange={(_, newValue) => setAnvisaInputValue(newValue)}
                                    loading={anvisaLoading}
                                    filterOptions={(x) => x}
                                    noOptionsText={
                                        anvisaInputValue.trim().length < 3
