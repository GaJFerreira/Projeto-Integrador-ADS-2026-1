            ...
            try {
                ...
                const itensVM: ListaItemVM[] =
                    lista.itens
                        ...
                        .map((it) => {
                            const pApi = it.produto as NonNullable<typeof it.produto>;
    ...
    const handleAdicionar = async () => {
        ...
        if (p.id > 0) {
            try {
                ...
                if (!isTemplate) {
                    // LISTA NORMAL → valida pelas patologias do usuário (backend pega do JWT)
                    substituiveis = await produtoService.listarSubstituiveis(p.id);
                                                        ...
                                                        <Box>
                                                            ...
                                                            {riscos.map((r) => (
                                                                <Box key={r.patologia.id}>
                                                                    ...
                                                                    <b>{r.produtoSugestao?.nome ?? 'sem sugestao cadastrada'}</b>
                                                                </Box>
                                                            ))}
                                                        </Box>
                                                    }
